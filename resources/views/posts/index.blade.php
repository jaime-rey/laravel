        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($posts as $post)
                <article class="w-full h-80 bg-cover bg-center @if ($loop->first) md:col-span-2 @endif"
                    style="background-image: url(@if($post->image) {{ env('APP_URL') . Storage::url($post->image->url) }} @else https://cdn.pixabay.com/photo/2015/05/22/05/52/cat-778315__340.jpg @endif)">
